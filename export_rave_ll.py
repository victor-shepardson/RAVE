...
class args(Config):
    ...
    CACHED = True
    # this is the number of latents to crop to
    DIM = 12
...
from rave.looper import LivingLooper
...
class TraceModel(nn.Module):
    def __init__(self, 
            pretrained: RAVE, resample: Resampling, looper: LivingLooper,
            dim: int):
        super().__init__()
    ...
    def fit_loop(self, x, y):
        return self.looper.fit_loop(x, y)

    @torch.jit.export
    def eval_loop(self, x):
        return self.looper.eval_loop(x)
...
looper = LivingLooper(args.CONTEXT*args.DIM*args.LOOPS, args.DIM)
...
model = TraceModel(model, resample, looper, args.DIM)
model(x)
